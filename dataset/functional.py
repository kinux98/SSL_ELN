    ...
    if _PIL_TO_TORCH_INTERP_MODE[interpolation] == "nearest":
        if img.dtype == torch.int64:
            out = torch.nn.functional.interpolate(
                img.float(), size=size, mode=_PIL_TO_TORCH_INTERP_MODE[interpolation]
            ).long()
        else:
            out = torch.nn.functional.interpolate(
                img, size=size, mode=_PIL_TO_TORCH_INTERP_MODE[interpolation]
            )
    else:
        out = torch.nn.functional.interpolate(
            img,
...
def pad(img, padding, fill=0, padding_mode="constant"):
    ...
    if isinstance(img, torch.Tensor):
        ...
        return torch.nn.functional.pad(
            input=img, pad=(l, r, t, b), mode=padding_mode, value=fill
        )
...
def adjust_gamma(img, gamma, gain=1):
    ...
    gamma_map = [255 * gain * pow(ele / 255.0, gamma) for ele in range(256)] * 3
...
def _get_inverse_affine_matrix(center, angle, translate, scale, shear):
    ...
    d = math.cos(angle + shear[0]) * math.cos(angle + shear[1]) + math.sin(
        angle + shear[0]
    ) * math.sin(angle + shear[1])
    matrix = [
        math.cos(angle + shear[0]),
        math.sin(angle + shear[0]),
        0,
        -math.sin(angle + shear[1]),
        math.cos(angle + shear[1]),
        0,
    ]
